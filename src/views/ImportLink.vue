...
<template>
    <WithSidebar>
        <div class="wrap">
            <div v-if="error == null" class="flex-col">
                <span>
                    <strong>{{ who }}</strong> wants to share:
                </span>
                <span v-if="what.length == 0" class="text-muted italic text-sm">Nothing</span>
                <div class="mt-4 w-max flex flex-col gap-4" v-else>
                    <div class="items">
                        <div v-for="item in what" class="card flex gap-2 items-center">
                    ...
                    <div class="buttons flex gap-2">
                        <IconButton type="action-green" icon="check" label="Accept" @click="confirm" />
                        <IconButton type="action-red" icon="close" label="Reject" @click="router.push('/')" />
                    </div>
                </div>
            </div>
            <div v-else-if="error == 'not-found'" class="error flex-col">
                ...
                <div class="flex gap-2">
                    <IconButton type="action-green" icon="add" label="Create New Course" @click="createNewCourse" />
                    <IconButton icon="home" label="Home" @click="router.push('/')" />
                </div>
            </div>
            <div v-else-if="error == 'not-allowed'" class="error flex-col">
                <h1>Not Allowed</h1>
                <p>You don't have permission to view this course.</p>
                <div class="card">
                    <p>Wait, this is my Gist!</p>
                    <input type="text" :value="githubPat" placeholder="Enter your GitHub PAT here" />
                    <IconButton type="action-green" icon="check" label="Accept" @click="tryAgain"
                        :disabled="!githubPat.length" />
                </div>
                <IconButton type="action-red" icon="close" label="Close" @click="router.push('/')" />
            </div>
        </div>
    </WithSidebar>
</template>
